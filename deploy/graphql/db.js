"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
var thumbnail = exports.thumbnail = {
  iBurn: "https://i.imgur.com/RbgXS4p.jpg"
};

var songs = exports.songs = [{
  name: "한(寒)",
  youtubeID: "HwC58-_VIpo",
  title: false,
  thumbnail: thumbnail.iBurn,
  album: "I burn"
}, {
  name: "화(火花)",
  youtubeID: "z3szNvgQxHo",
  title: true,
  thumbnail: thumbnail.iBurn,
  album: "I burn"
}, {
  name: "MOON",
  youtubeID: "OYtjo3CTAOM",
  title: false,
  thumbnail: thumbnail.iBurn,
  album: "I burn"
}, {
  name: "Where is love",
  youtubeID: "L4WnbelD60E",
  title: false,
  thumbnail: thumbnail.iBurn,
  album: "I burn"
}, {
  name: "LOST",
  youtubeID: "ILmHLmZiVG0",
  title: false,
  thumbnail: thumbnail.iBurn,
  album: "I burn"
}, {
  name: "DAHLIA",
  youtubeID: "adl0yFg8Ntc",
  title: false,
  thumbnail: thumbnail.iBurn,
  album: "I burn"
}, {
  name: "Oh my god",
  youtubeID: "om3n2ni8luE",
  title: true,
  thumbnail: "https://i.imgur.com/2vm2WEj.jpg",
  album: "I trust"
}, {
  name: "사랑해",
  youtubeID: "TzepUKHa9lg",
  title: false,
  thumbnail: "https://i.imgur.com/2vm2WEj.jpg",
  album: "I trust"
}, {
  name: "Maybe",
  youtubeID: "k8pEDdNUvzI",
  title: false,
  thumbnail: "https://i.imgur.com/2vm2WEj.jpg",
  album: "I trust"
}, {
  name: "LION",
  youtubeID: "6oanIo_2Z4Q",
  title: false,
  thumbnail: "https://i.imgur.com/2vm2WEj.jpg",
  album: "I trust"
}, {
  name: "Oh my god (English Ver.)",
  youtubeID: "UR-1Tgx1MeA",
  title: false,
  thumbnail: "https://i.imgur.com/2vm2WEj.jpg",
  album: "I trust"
}, {
  name: "Senorita",
  youtubeID: "G8APgeFfkAk",
  title: true,
  thumbnail: "https://i.imgur.com/kYjWW9l.jpg",
  album: "I made"
}, {
  name: "What's Your Name",
  youtubeID: "v8zTRXCgF1Y",
  title: false,
  thumbnail: "https://i.imgur.com/kYjWW9l.jpg",
  album: "I made"
}, {
  name: "싫다고 말해",
  youtubeID: "MIAtuJ31l98",
  title: false,
  thumbnail: "https://i.imgur.com/kYjWW9l.jpg",
  album: "I made"
}, {
  name: "주세요",
  youtubeID: "HmUlhIjkPYE",
  title: false,
  thumbnail: "https://i.imgur.com/kYjWW9l.jpg",
  album: "I made"
}, {
  name: "Blow Your Mind",
  youtubeID: "Q8RQiYxsQGk",
  title: false,
  thumbnail: "https://i.imgur.com/kYjWW9l.jpg",
  album: "I made"
}, {
  name: "LATATA",
  youtubeID: "9mQk7Evt6Vs",
  title: true,
  thumbnail: "https://i.imgur.com/Eo3K495.jpg",
  album: "I am"
}, {
  name: "달라 ($$$)",
  youtubeID: "sTnQ24z9hIw",
  title: false,
  thumbnail: "https://i.imgur.com/Eo3K495.jpg",
  album: "I am"
}, {
  name: "MAZE",
  youtubeID: "klKKrmRY2R8",
  title: false,
  thumbnail: "https://i.imgur.com/Eo3K495.jpg",
  album: "I am"
}, {
  name: "DON'T TEXT ME",
  youtubeID: "9qXLfXLluQI",
  title: false,
  thumbnail: "https://i.imgur.com/Eo3K495.jpg",
  album: "I am"
}, {
  name: "알고 싶어",
  youtubeID: "ehXwFRCvTa8",
  title: false,
  thumbnail: "https://i.imgur.com/Eo3K495.jpg",
  album: "I am"
}, {
  name: "들어줘요",
  youtubeID: "OVivuUWG9Qs",
  title: false,
  thumbnail: "https://i.imgur.com/Eo3K495.jpg",
  album: "I am"
}, {
  name: "DUMDi DUMDi",
  youtubeID: "HPQ5mqovXHo",
  title: true,
  thumbnail: "https://i.imgur.com/NkTiZy3.jpg",
  album: "DUMDi DUMDi"
}, {
  name: "i'M THE TREND",
  youtubeID: "ZlKoeJ3xlx4",
  title: true,
  thumbnail: "https://i.imgur.com/7bFy8bI.jpg",
  album: "i'M THE TREND"
}, {
  name: "Uh-Oh",
  youtubeID: "ycYLPbtxU1Q",
  title: true,
  thumbnail: "https://i.imgur.com/mBLwlbv.jpg",
  album: "Uh-Oh"
}, {
  name: "한(一)",
  youtubeID: "3K38Fc1SV5c",
  title: true,
  thumbnail: "https://i.imgur.com/CDr57eB.jpg",
  album: "한(一)"
}];

var albums = exports.albums = [{
  name: "I burn",
  img: "https://i.imgur.com/MaJtviQ.jpg",
  type: "mini",
  release: {
    year: "2021",
    month: "1",
    date: "11"
  }
}, {
  name: "I trust",
  img: "https://i.imgur.com/mOXhDp8.jpg",
  type: "mini",
  release: {
    year: "2020",
    month: "4",
    date: "6"
  }
}, {
  name: "I made",
  img: "https://i.imgur.com/x3VwXP6.jpg",
  type: "mini",
  release: {
    year: "2019",
    month: "2",
    date: "26"
  }
}, {
  name: "I am",
  img: "https://i.imgur.com/BAwiwGw.jpg",
  type: "mini",
  release: {
    year: "2018",
    month: "5",
    date: "2"
  }
}, {
  name: "DUMDi DUMDi",
  img: "https://i.imgur.com/hhbdydl.jpg",
  type: "single",
  release: {
    year: "2020",
    month: "8",
    date: "3"
  }
}, {
  name: "i'M THE TREND",
  img: "https://i.imgur.com/U4eLuYU.jpg",
  type: "single",
  release: {
    year: "2020",
    month: "7",
    date: "7"
  }
}, {
  name: "Uh-Oh",
  img: "https://i.imgur.com/ziT2Zl3.jpg",
  type: "single",
  release: {
    year: "2019",
    month: "6",
    date: "26"
  }
}, {
  name: "한(一)",
  img: "https://i.imgur.com/6ISk4m2.jpg",
  type: "single",
  release: {
    year: "2018",
    month: "8",
    date: "14"
  }
}];

var getSong = exports.getSong = function getSong(youtubeID) {
  return songs.find(function (song) {
    return song.youtubeID === youtubeID;
  });
};
var getSongs = exports.getSongs = function getSongs(album) {
  return songs.filter(function (song) {
    return song.album === album;
  });
};